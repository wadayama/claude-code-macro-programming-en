...
## Audit Log & Decision Reasoning Recording System

### 🚨 Absolute Compliance Rule: Decision Reasoning Recording

When receiving decision reasoning recording instructions "Record 'DECISION' with reasoning 'REASONING' to log":

1. **Always execute audit log recording using Bash tool**
   ```bash
   uv run python -c "from audit_logger import log_decision; log_decision('DECISION', 'REASONING'); print('Recorded decision reasoning to log')"
   ```

2. **Recording Processing Specifications**
   - Permanently save decision content and reasoning to SQLite database
   - Automatically record timestamp and event type
   - Ensure transparency and explainability

3. **Always report recording completion**
   - Display "Recorded decision reasoning to log"

### 🚨 Absolute Compliance Rule: Reasoning Process Recording

When receiving reasoning process recording instructions "Record reasoning 'REASONING' with result 'RESULT' in context 'CONTEXT' to log":

1. **Always execute reasoning log recording using Bash tool**
   ```bash
   uv run python -c "from audit_logger import log_reasoning; log_reasoning('CONTEXT', 'REASONING', 'RESULT'); print('Recorded reasoning process to log')"
   ```

2. **Recording Processing Specifications**
   - Save reasoning context, process, and result in structured format
   - Make LLM thought process completely traceable
   - Integration with Pattern 10 Debug & Tracing

3. **Always report recording completion**
   - Display "Recorded reasoning process to log"

### Execution Examples

```
# Decision reasoning recording example
User: "Since {{score}} is 85, I judged it as excellent. Record this decision reasoning to log"
AI execution:
1. Bash tool execution: uv run python -c "from audit_logger import log_decision; log_decision('Judged as excellent because {{score}}(85)', 'Meets excellent criteria as 80+ points'); print('Recorded decision reasoning to log')"
2. Save decision and reasoning to SQLite database
3. Display: "Recorded decision reasoning to log"

# Reasoning process recording example
User: "Record the reasoning process from score 85 to excellent judgment in learning evaluation to log"
AI execution:
1. Bash tool execution: uv run python -c "from audit_logger import log_reasoning; log_reasoning('Learning evaluation system', 'Classified as excellent category because score 85 >= 80 condition is met', 'Excellent judgment'); print('Recorded reasoning process to log')"
2. Save reasoning context, process, and result to SQLite database
3. Display: "Recorded reasoning process to log"
```
...
- After variable operations, state and audit logs can be checked with `audit_viewer.py`
- Variable history is automatically recorded with timestamps
- Unicode (Japanese) strings are fully supported
- All operations are recorded transparently through A.6 audit log system