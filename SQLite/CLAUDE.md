...
1. **Always execute SQLite clear all variables using Bash tool**
   ```bash
...
3. **Always report completion of clearing**
   - Display "[deletion count] variables cleared"
...
AI execution:
1. Bash tool execution: uv run python -c "from variable_db import save_variable; save_variable('user_name', 'Tanaka Taro'); print('Saved \"Tanaka Taro\" to {{user_name}}')"
2. Storage completed in SQLite database
3. Display: "Saved 'Tanaka Taro' to {{user_name}}"
...
- Variable states can be verified with `watch_variables.py` after variable operations
