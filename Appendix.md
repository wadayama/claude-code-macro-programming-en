...
### Runtime Validation Strategy and Error Handling

Schema validation can impact performance if performed every execution, making appropriate timing and error response strategies crucial.

#### Validation Timing Strategy Selection

**1. Immediate Validation Mode** (High Safety, Low Performance):
```python
def safe_load_variable(key):
    """Execute schema validation every time when loading variables"""
    data = validate_and_load_variables()  # Validate every time
    if data is None:
        return handle_validation_error(key)
    return data.get(key)
```

**2. Periodic Validation Mode** (Balanced, Recommended):
```python
import time

last_validation_time = 0
VALIDATION_INTERVAL = 300  # 5-minute intervals

def periodic_load_variable(key):
    """Execute schema validation periodically"""
    ...
    if current_time - last_validation_time > VALIDATION_INTERVAL:
        ...
        # Normal loading (skip validation)
        with open("variables.json", 'r', encoding='utf-8') as f:
            data = json.load(f)
...
**3. Conditional Validation Mode** (High Performance, Risk Management):
```python
def conditional_load_variable(key, force_validation=False):
    """Execute schema validation only for critical operations"""
    critical_keys = ["api_endpoint", "security_token", "calculation_precision"]
    
    if force_validation or key in critical_keys:
        data = validate_and_load_variables()
        if data is None:
            return handle_validation_error(key)
    else:
...
Staged fallback processing for validation errors:

```python
def handle_validation_error(key, fallback_strategy="default"):
    """Staged fallback processing for validation errors"""
    
    if fallback_strategy == "default":
        # Step 1: Continue with default values
        default_values = {
            "user_name": "anonymous",
            "analysis_precision": 0.8,
            "iteration_count": 10
        }
        if key in default_values:
            print(f"Warning: {key} schema validation failed, using default value")
            return default_values[key]
    
    elif fallback_strategy == "backup":
        # Step 2: Recovery from backup file
        try:
            with open("variables.backup.json", 'r', encoding='utf-8') as f:
                backup_data = json.load(f)
            print(f"Warning: {key} recovered from backup")
            return backup_data.get(key)
        except:
            pass
    
    elif fallback_strategy == "repair":
        # Step 3: Attempt automatic repair
        try:
            repaired_data = auto_repair_variables()
            if repaired_data:
                print(f"Info: {key} automatic data repair completed")
                return repaired_data.get(key)
        except:
            pass
    
    # Step 4: Safe shutdown
    print(f"Error: Failed to retrieve {key}, safely stopping processing")
    return None

def auto_repair_variables():
    """Automatic data repair based on schema"""
    try:
        # Load schema
        with open("variables.schema.json", 'r', encoding='utf-8') as f:
            schema = json.load(f)
        ...
        for key, value in data.items():
            if key in schema.get("properties", {}):
                ...
                if expected_type == "number" and isinstance(value, str):
                    try:
                        repaired_data[key] = float(value)
                    except:
                        repaired_data[key] = 0.0
                elif expected_type == "integer" and isinstance(value, float):
                    repaired_data[key] = int(value)
                else:
...
**Validation Frequency Adjustment Principles**:
- **Mission Critical**: Immediate validation (every time)
- **Business Applications**: Periodic validation (5-15 minute intervals)
- **Prototypes/Experiments**: Conditional validation (critical data only)

**Resource Usage Management**:
- Memory caching of schema files
- Reduced processing load through partial validation
- Maintained responsiveness through asynchronous validation

This runtime strategy enables optimal balance between schema validation safety and performance.
