...
### 🚨 Absolute Compliance Rule: Typed Variable Storage

When receiving typed variable storage instructions "Save VALUE as TYPE type to {{variable_name}}":

1. **Always execute typed SQLite variable storage using Bash tool**
   ```bash
   uv run python -c "from variable_db import save_variable; save_variable('variable_name', 'VALUE', 'TYPE'); print('Saved \"VALUE\" to {{variable_name}} as TYPE type')"
   ```

2. **Typed Storage Processing Specifications**
   - Execute value type validation (type conversion, range check, enumeration check)
   - Save only if validation succeeds
   - Save type information along with value
   - Raise SchemaValidationError if validation fails

3. **Available Types**
   - **Basic types**: integer, number, string, boolean
   - **Constrained types**: age(0-150), percentage(0-100), status(pending/completed/failed)

4. **Always report storage completion or failure**
   - Success: "Saved \"VALUE\" to {{variable_name}} as TYPE type"
...
### 🚨 Absolute Compliance Rule: Type Check Execution

When receiving type check instructions "Execute type check for all variables":

1. **Always execute type check using Bash tool**
   ```bash
   uv run python -c "from variable_db import validate_all; results = validate_all(); print('Type check results:'); [print(f'{name}: {\"Valid\" if result is True else result}') for name, result in results.items()]"
   ```

2. **Type Check Processing Specifications**
   - Validate all variables with type information set
   - Exclude untyped variables from validation
   - Compare current value of each variable against corresponding type definition
   - Display validation results as a list

3. **Always report validation results**
   - Display "Valid" or specific error message for each variable
...
# Typed variable storage example
User: "Save 25 as integer type to {{user_age}}"
AI execution:
1. Bash tool execution: uv run python -c "from variable_db import save_variable; save_variable('user_age', '25', 'integer'); print('Saved \"25\" to {{user_age}} as integer type')"
2. Execute type validation (integer conversion)
3. Save to SQLite database with type information completed
4. Display: "Saved \"25\" to {{user_age}} as integer type"

# Constrained type storage example
User: "Save 30 as age type to {{current_age}}"
AI execution:
1. Bash tool execution: uv run python -c "from variable_db import save_variable; save_variable('current_age', '30', 'age'); print('Saved \"30\" to {{current_age}} as age type')"
2. Execute type validation (integer conversion + 0-150 range check)
3. Save to SQLite database with type information completed
4. Display: "Saved \"30\" to {{current_age}} as age type"

# Type validation error example
User: "Save -5 as age type to {{invalid_age}}"
AI execution:
1. Bash tool execution: uv run python -c "from variable_db import save_variable; save_variable('invalid_age', '-5', 'age'); print('Saved \"-5\" to {{invalid_age}} as age type')"
2. Type validation error occurs (range constraint violation)
3. SchemaValidationError: Value -5 is below minimum 0 for type age
4. Display error message

# Type check execution example
User: "Execute type check for all variables"
AI execution:
1. Bash tool execution: uv run python -c "from variable_db import validate_all; results = validate_all(); print('Type check results:'); [print(f'{name}: {\"Valid\" if result is True else result}') for name, result in results.items()]"
2. Execute validation for variables with type information
